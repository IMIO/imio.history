...
class TestContentHistory(FunctionalTestCase):
    ...
    def test_getTransitionTitle(self):
        ...
        self.assertEqual(
            view.getTransitionTitle("publish"), "Reviewer publishes content"
        )
        # if the transition does not exist, it will return passed transition id
        self.assertEqual(
            view.getTransitionTitle("unexisting_transition_id"),
        ...
        self.assertEqual(view.getTransitionTitle("publish"), "publish")
        # an entry that is not a transition_id but contains special chars does not break
        self.assertEqual(view.getTransitionTitle("sp\xe9cial"), "sp\xe9cial")
        self.assertEqual(view.getTransitionTitle("sp\xc3\xa9cial"), "sp\xc3\xa9cial")
    ...
    def test_showRevisionInfos(self):
        ...
        pr.setVersionableContentType(
            ["ATDocument", "ATNewsItem", "Event", "Link", "News Item"]
        )
    ...
    def test_renderComments(self):
        ...
        self.wft.doActionFor(
            self.doc,
            "retract",
            comment="Custom comments not translatable.\nAnd one additional line.",
        )
        last_event = view.getHistory()[0]
        self.assertEqual(
            view.renderComments(last_event),
        ...
        self.assertEqual(
            view.renderComments(last_event), "<p>*** x-patch mimetype ***</p>"
        )
