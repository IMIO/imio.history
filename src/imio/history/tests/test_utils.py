...
class TestUtils(IntegrationTestCase):
    ...
    def test_getLastAction_checkMayView(self):
        ...
        adapter = getAdapter(doc, IImioHistory, "workflow")
        self.assertEqual(
            getLastAction(adapter, checkMayViewEvent=False, checkMayViewComment=False)[
                "action"
            ],
            "publish",
        )
        self.assertEqual(
            getLastAction(adapter, checkMayViewEvent=True, checkMayViewComment=True)[
                "action"
            ],
            "publish",
        )
    ...
    def test_getLastWFAction(self):
        ...
        self.assertEqual(
            getLastWFAction(doc, transition="before_last")["review_state"], "private"
        )
    ...
    def test_getLastWFAction_ignore_previous_event_actions(self):
        ...
        self.assertEqual(
            action_with_ignored_previous["comments"], "Publication comment"
        )
    ...
    def test_get_all_history_attr(self):
        ...
        self.assertEqual(
            get_all_history_attr(doc, attr_name="review_state"),
            ["private", "published", "private"],
        )
        self.assertEqual(
            get_all_history_attr(doc, attr_name="actor"),
            ["test_user_1_", "test_user_1_", "test_user_1_"],
        )
        self.assertEqual(
            get_all_history_attr(doc, attr_name="comments"),
            ["", "Publication comment", ""],
        )
    ...
    def test_get_event_by_time(self):
        ...
        api.content.transition(doc, "publish", comment="Publication comment")
        api.content.transition(doc, "retract")
        float_event_time = float(
            doc.workflow_history["simple_publication_workflow"][-1]["time"]
        )
        self.assertEqual(
            get_event_by_time(doc, "workflow", float_event_time)["action"], "retract"
        )
    ...
    def test_add_event_to_wf_history(self):
        ...
        self.assertEqual(
            doc.workflow_history["simple_publication_workflow"][-1]["action"], "test"
        )
        # insert at index 1
        ...
        self.assertEqual(
            doc.workflow_history["simple_publication_workflow"][-2]["action"], "test2"
        )
