...
def getPreviousEvent(
    ...
    history = adapter.getHistory(checkMayViewEvent=False, checkMayViewComment=False)
    res = None
...
def getLastAction(
    ...
    if action == "last":
        ...
        while i >= 0:
            ...
            if condition and (
                not ignore_previous_event_actions
                or history[i - 1]["action"] not in ignore_previous_event_actions
            ):
                res = event
                break
            i -= 1
...
def get_all_history_attr(
    ...
    res = [
        event[attr_name]
        for event in adapter.getHistory(checkMayViewEvent, checkMayViewComment)
    ]
    return res


def add_event_to_history(
    obj, history_attr, action, actor=None, time=None, comments="", extra_infos={}
):
